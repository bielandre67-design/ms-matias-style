...
function prepararOpcoesFreteMS(fretes) {
  const validos = (Array.isArray(fretes) ? fretes : []).filter(frete => {
    ...
    if (frete.custom === true || frete.free === true) return true;
...
(function(){
  ...
  function cardRecomendadoBancoMS(produto){
    ...
    return `<article class="recomendado-card-ms recomendado-banco-ms"
      ...
      onclick="return window.abrirProdutoRecomendadoBancoMS(event, this)"
  ...
  document.addEventListener('click', evento => {
    if (evento.target instanceof Element && evento.target.closest('.card-produto')){
      setTimeout(() => {
  ...
  window.abrirProdutoRecomendadoBancoMS = function(evento, cardRecomendado){
    if(evento?.target?.closest?.('.rec-fav-ms')) return false;

    evento?.preventDefault?.();
    evento?.stopPropagation?.();

    const id = String(cardRecomendado?.dataset?.idBanco || '');
    ...
    if(!produto){
      console.warn('Produto recomendado não encontrado no catálogo:', id);
      return false;
    }
    ...
    const cores = Array.isArray(produto.cores) ? produto.cores : [];
    const preco = numeroPrecoMS(produto.preco);
    const antigo = produto.precoAntigo == null ? '' : numeroPrecoMS(produto.precoAntigo);

    const cardCompleto = document.createElement('div');
    cardCompleto.className = 'card-produto produto-banco-ms';
    cardCompleto.dataset.idBanco = id;
    cardCompleto.dataset.nome = produto.nome || 'Produto MS';
    cardCompleto.dataset.preco = preco.toFixed(2);
    cardCompleto.dataset.precoantigo = antigo === '' ? '' : antigo.toFixed(2);
    cardCompleto.dataset.img = fotos[0] || 'logo.png';
    cardCompleto.dataset.fotos = fotos.join(',');
    cardCompleto.dataset.descricao = produto.tabelaMedidas || produto.descricao || '';
    cardCompleto.dataset.detalhes = produto.detalhesProduto || '';
    cardCompleto.dataset.composicao = produto.composicao || '';
    cardCompleto.dataset.cuidados = produto.cuidados || '';
    cardCompleto.dataset.cores = cores.join(',');
    cardCompleto.dataset.tamanhos = tamanhos.join(',');
    cardCompleto.dataset.categoria = produto.categoria || '';
    ...
    if(typeof window.abrirProdutoDetalheCard === 'function'){
      ...
      abrirProdutoDetalheCard(cardCompleto);
    }else{
      console.error('Função de abrir detalhes não encontrada.');
      return false;
    }

    setTimeout(() => {
      if(typeof window.sincronizarTamanhosProdutoMS === 'function'){
        window.sincronizarTamanhosProdutoMS(cardCompleto);
      }
      const detalhe = document.getElementById('produtoDetalhe');
      if(detalhe){
        detalhe.classList.add('ativo');
  ...
  document.addEventListener('keydown', function(evento){
    ...
    window.abrirProdutoRecomendadoBancoMS(evento, card);
  });
...
(function(){
  ...
  function cssHome(){
    if(document.getElementById('cssDestaquesHomeMS'))return;
    const s=document.createElement('style');s.id='cssDestaquesHomeMS';
    s.textContent=`
      #destaquesHomeMS.destaques-home-ms{position:relative;padding:58px 4% 64px;background:linear-gradient(180deg,#0b0b0c 0%,#111214 100%);color:#fff;border-top:1px solid rgba(218,169,46,.32);border-bottom:1px solid rgba(218,169,46,.22);overflow:hidden}
      #destaquesHomeMS.destaques-home-ms:before{content:"";position:absolute;inset:auto -10% -65% 35%;height:260px;background:radial-gradient(circle,rgba(218,169,46,.16),transparent 66%);pointer-events:none}
      #destaquesHomeMS .destaques-home-topo-ms{position:relative;display:flex;align-items:end;justify-content:space-between;gap:15px;max-width:1500px;margin:0 auto 28px}
      #destaquesHomeMS .destaques-home-topo-ms h2{margin:0;color:#fff;font-size:clamp(1.55rem,3vw,2.45rem);letter-spacing:.08em;text-transform:uppercase}
      #destaquesHomeMS .destaques-home-topo-ms h2:after{content:"";display:block;width:74px;height:3px;margin-top:12px;border-radius:99px;background:#d9aa35}
      #destaquesHomeMS .destaques-home-grade-ms{position:relative;max-width:1500px;margin:0 auto;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:22px}
      #destaquesHomeMS .destaque-home-card-ms{min-width:0;overflow:hidden;cursor:pointer;background:linear-gradient(145deg,#17181b,#101114);border:1px solid rgba(218,169,46,.24);border-radius:20px;padding:0 0 18px;color:#fff;box-shadow:0 18px 40px rgba(0,0,0,.34);transition:transform .22s ease,border-color .22s ease,box-shadow .22s ease}
      #destaquesHomeMS .destaque-home-card-ms:hover{transform:translateY(-5px);border-color:rgba(218,169,46,.72);box-shadow:0 24px 55px rgba(0,0,0,.46)}
      #destaquesHomeMS .destaque-home-card-ms img{width:100%;aspect-ratio:4/5;object-fit:cover;display:block;background:#202124;border-radius:19px 19px 0 0}
      #destaquesHomeMS .destaque-home-card-ms h3{font-size:1rem;line-height:1.35;margin:16px 16px 7px;color:#fff}
      #destaquesHomeMS .destaque-home-card-ms strong{display:block;margin:0 16px;color:#e3b73f;font-size:1.12rem}
      #destaquesHomeMS .destaque-home-card-ms p{display:inline-flex;margin:13px 16px 0;padding:9px 13px;border:1px solid rgba(218,169,46,.38);border-radius:999px;color:#f2d577;font-size:.83rem;font-weight:700}
      @media(max-width:1000px){#destaquesHomeMS .destaques-home-grade-ms{grid-template-columns:repeat(3,minmax(0,1fr))}}
      @media(max-width:760px){#destaquesHomeMS.destaques-home-ms{padding:38px 14px 44px}#destaquesHomeMS .destaques-home-grade-ms{grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}#destaquesHomeMS .destaque-home-card-ms{border-radius:15px;padding-bottom:14px}#destaquesHomeMS .destaque-home-card-ms img{border-radius:14px 14px 0 0}#destaquesHomeMS .destaque-home-card-ms h3{margin:12px 11px 5px;font-size:.9rem}#destaquesHomeMS .destaque-home-card-ms strong{margin:0 11px;font-size:1rem}#destaquesHomeMS .destaque-home-card-ms p{margin:10px 11px 0;padding:7px 10px;font-size:.75rem}}
    `;
