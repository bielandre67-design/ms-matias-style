(function(){
  ...
  function render(){
    ...
    lista.innerHTML=(config.itens||[]).sort((a,b)=>(a.ordem||0)-(b.ordem||0)).map((item,i)=>`<article class="beneficio-editor-item-ms" data-beneficio-index="${i}">
      ...
      <label>Ícone<input class="beneficio-icone-ms" value="${esc(item.icone||'✓')}" maxlength="12"></label>
  ...
  function lerTela(){
    const itens=[...document.querySelectorAll('[data-beneficio-index]')].map((el,i)=>({
      ...
      icone:el.querySelector('.beneficio-icone-ms').value.trim()||'✓',
  ...
  function atualizarPreview(){
    ...
    p.innerHTML=(config.itens||[]).filter(x=>x.ativo!==false).sort((a,b)=>a.ordem-b.ordem).map(x=>`<span>${esc(x.icone)} ${esc(x.texto)}</span>`).join('');
  }
  ...
  window.adicionarBeneficioMS=function(){
    ...
    config.itens.push({id:`beneficio-${Date.now()}`,icone:'✓',texto:'NOVO BENEFÍCIO',ativo:true,ordem:config.itens.length+1});render();
  };
