    ...
    sec.innerHTML=`
      ...
      <form id="formProdutoMS" class="ms-prod-form" style="display:none">
        ...
        <div class="ms-upload-box">
          ...
          <div id="produtoPreviewMS" class="ms-preview-grid"></div><div id="produtoFotosContadorMS" class="ms-upload-count">0 de 12 fotos</div>
        </div>
...
M: busto 55 cm | comprimento 70 cm"></textarea></label><label>Detalhes do produto<textarea id="produtoDetalhesMS" placeholder="Modelagem, acabamento, bolsos, capuz e outros detalhes."></textarea></label><label>Composição<textarea id="produtoComposicaoMS" placeholder="Ex.: 50% algodão e 50% poliéster."></textarea></label><label>Cuidados com a peça<textarea id="produtoCuidadosMS" placeholder="Ex.: lavar do avesso, não usar alvejante e secar à sombra."></textarea></label></div></div>
  ...
  function atualizarContadorFotosMS(){
    const contador=document.getElementById('produtoFotosContadorMS');
    if(contador) contador.textContent=`${imagensSelecionadasMS.length} de 12 fotos · arraste para ordenar`;
  }
  function moverImagemMS(origem,destino){
    origem=Number(origem); destino=Number(destino);
    if(!Number.isInteger(origem)||!Number.isInteger(destino)||origem===destino||origem<0||destino<0||origem>=imagensSelecionadasMS.length||destino>=imagensSelecionadasMS.length)return;
    const [item]=imagensSelecionadasMS.splice(origem,1);
    imagensSelecionadasMS.splice(destino,0,item);
    renderPreviewMS();
  }
  function renderPreviewMS(){
    const box=document.getElementById('produtoPreviewMS'); if(!box)return;
    atualizarContadorFotosMS();
    if(!imagensSelecionadasMS.length){box.innerHTML='<div class="ms-upload-help">Nenhuma foto selecionada.</div>';return;}
    box.innerHTML=imagensSelecionadasMS.map((item,i)=>`<div class="ms-preview" draggable="true" data-indice="${i}" title="Arraste para mudar a ordem"><img src="${esc(item.url)}" alt="Foto ${i+1} do produto"><span class="ms-preview-main">${i===0?'PRINCIPAL':`FOTO ${i+1}`}</span><div class="ms-preview-actions"><button class="ms-preview-order" type="button" title="Mover para a esquerda" onclick="moverImagemBotaoMS(${i},-1)">‹</button><button type="button" onclick="principalImagemMS(${i})">${i===0?'Principal':'Tornar principal'}</button><button class="ms-preview-order" type="button" title="Mover para a direita" onclick="moverImagemBotaoMS(${i},1)">›</button><button class="danger" type="button" title="Excluir foto" onclick="removerImagemMS(${i})">×</button></div></div>`).join('');
    box.querySelectorAll('.ms-preview').forEach(card=>{
      card.addEventListener('dragstart',e=>{e.dataTransfer.effectAllowed='move';e.dataTransfer.setData('text/plain',card.dataset.indice);card.classList.add('ms-arrastando');});
      card.addEventListener('dragend',()=>{box.querySelectorAll('.ms-preview').forEach(x=>x.classList.remove('ms-arrastando','ms-alvo'));});
      card.addEventListener('dragover',e=>{e.preventDefault();e.dataTransfer.dropEffect='move';card.classList.add('ms-alvo');});
      card.addEventListener('dragleave',()=>card.classList.remove('ms-alvo'));
      card.addEventListener('drop',e=>{e.preventDefault();card.classList.remove('ms-alvo');moverImagemMS(e.dataTransfer.getData('text/plain'),card.dataset.indice);});
    });
  }
  window.moverImagemBotaoMS=function(i,direcao){moverImagemMS(i,i+direcao);};
  window.principalImagemMS=function(i){if(i===0)return;const [x]=imagensSelecionadasMS.splice(i,1);imagensSelecionadasMS.unshift(x);renderPreviewMS();};
