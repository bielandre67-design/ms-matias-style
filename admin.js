...
M: busto 55 cm | comprimento 70 cm"></textarea></label><label>Detalhes do produto<textarea id="produtoDetalhesMS" placeholder="Modelagem, acabamento, bolsos, capuz e outros detalhes."></textarea></label><label>Composição<textarea id="produtoComposicaoMS" placeholder="Ex.: 50% algodão e 50% poliéster."></textarea></label><label>Cuidados com a peça<textarea id="produtoCuidadosMS" placeholder="Ex.: lavar do avesso, não usar alvejante e secar à sombra."></textarea></label></div></div>
        <label>Cores, separadas por vírgula<input id="produtoCoresMS" placeholder="Preto, Bege, Rosa"></label>
        <div class="ms-variacoes-box"><div class="ms-variacoes-top"><div><strong>📐 Tamanhos e numerações</strong><div class="ms-prod-meta">Escolha um modelo pronto ou digite opções personalizadas.</div></div><div class="ms-variacoes-presets"><button type="button" onclick="usarVariacoesProdutoMS('roupas')">P · M · G · GG</button><button type="button" onclick="usarVariacoesProdutoMS('numeros')">34 até 44</button><button type="button" onclick="usarVariacoesProdutoMS('unico')">Único</button><button type="button" onclick="usarVariacoesProdutoMS('limpar')">Limpar</button></div></div><label>Opções separadas por vírgula<input id="produtoTamanhosMS" value="P, M, G, GG" placeholder="P, M, G, GG ou 34, 35, 36..."></label><div id="previewVariacoesProdutoMS" class="ms-variacoes-preview">Serão criadas: <strong>P, M, G e GG</strong></div></div>
  ...
  function listaVariacoesProdutoMS(){
    return String(document.getElementById('produtoTamanhosMS')?.value||'')
      .split(',').map(x=>x.trim().toUpperCase()).filter(Boolean)
      .filter((v,i,a)=>a.indexOf(v)===i);
  }
  window.usarVariacoesProdutoMS=function(tipo){
    const input=document.getElementById('produtoTamanhosMS'); if(!input)return;
    if(tipo==='roupas') input.value='P, M, G, GG';
    else if(tipo==='numeros') input.value=Array.from({length:11},(_,i)=>String(34+i)).join(', ');
    else if(tipo==='unico') input.value='ÚNICO';
    else if(tipo==='limpar') input.value='';
    atualizarPreviewVariacoesProdutoMS(); input.focus();
  };
  function atualizarPreviewVariacoesProdutoMS(){
    const el=document.getElementById('previewVariacoesProdutoMS'); if(!el)return;
    const lista=listaVariacoesProdutoMS();
    el.innerHTML=lista.length?`Serão criadas no estoque: <strong>${lista.map(esc).join(', ')}</strong>`:'Digite pelo menos um tamanho ou número.';
  }
  async function garantirVariacoesEstoqueProdutoMS(body, quantidadeInicial, produtoNovo){
    const cores=body.cores.length?body.cores:['Única'];
    const tamanhos=body.tamanhos.length?body.tamanhos:['ÚNICO'];
    let existentes=[];
    try{const r=await fetch(`${API}/estoque?t=${Date.now()}`,{cache:'no-store'});if(r.ok)existentes=await r.json();}catch{}
    const chave=(nome,cor,tamanho)=>[nome,cor,tamanho].map(v=>String(v||'').trim().toLowerCase()).join('|');
    const mapa=new Set((Array.isArray(existentes)?existentes:[]).map(i=>chave(i.nome,i.cor,i.tamanho)));
    const tarefas=[];
    for(const cor of cores){for(const tamanho of tamanhos){
      const k=chave(body.nome,cor,tamanho);
      if(!mapa.has(k)){
        tarefas.push(fetch(`${API}/estoque`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({nome:body.nome,cor,tamanho,quantidade:produtoNovo?quantidadeInicial:0})}));
      }
    }}
    if(tarefas.length){const respostas=await Promise.all(tarefas);if(respostas.some(r=>!r.ok))throw new Error('Produto salvo, mas não foi possível criar todas as opções no estoque.');}
    return tarefas.length;
  }
  window.novoProdutoMS=async function(){ document.getElementById('formProdutoMS').reset(); produtoIdMS.value=''; produtoTamanhosMS.value='P, M, G, GG'; if(!categoriasMS.length) await carregarCategoriasMS(); produtoCategoriaMS.value=categoriasMS[0]?.nome||'Roupas'; imagensSelecionadasMS=[]; renderPreviewMS(); atualizarStatusMedidasMS(); produtoAtivoMS.checked=true; document.getElementById('tituloFormProdutoMS').textContent='Novo produto'; document.getElementById('modoFormProdutoMS').textContent='NOVO'; formProdutoMS.style.display='grid'; atualizarPreviewProdutoAoVivoMS(); atualizarPreviewVariacoesProdutoMS(); formProdutoMS.scrollIntoView({behavior:'smooth',block:'start'}); setTimeout(()=>produtoNomeMS.focus(),350); };
  ...
  document.addEventListener('input',e=>{if(['produtoNomeMS','produtoPrecoMS','produtoCategoriaMS','produtoAtivoMS'].includes(e.target?.id))atualizarPreviewProdutoAoVivoMS();if(e.target?.id==='produtoTamanhosMS')atualizarPreviewVariacoesProdutoMS();});
  ...
  async function salvarProdutoMS(ev){
    ...
    try{
      ...
      const qtd=Math.max(0,num(produtoQuantidadeMS.value));const criadas=await garantirVariacoesEstoqueProdutoMS(body,qtd,!id);
      cancelarProdutoMS();msg(criadas?`Produto salvo e ${criadas} opção${criadas===1?'':'ões'} criada${criadas===1?'':'s'} automaticamente no estoque.`:'Produto salvo. As opções já estavam cadastradas no estoque.');await carregarProdutosMS();
