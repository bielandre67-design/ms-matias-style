...
async function criarPreferenciaMP({ carrinhoItems, valorFrete, freteSelecionado, idPedido, percentualDesconto = 0 }) {
  ...
  if(configPagamento.cartaoAtivo === false){ const erro=new Error("O pagamento com cartão está desativado no painel administrativo."); erro.statusCode=400; throw erro; }
...
app.post("/criar-pagamento-pix", async (req, res) => {
  try {
    garantirMercadoPagoConfigurado();
    const configPagamento = await carregarConfigPagamentoServidorMS();
    if(configPagamento.pixAtivo === false) return res.status(400).json({erro:true,mensagem:"O PIX está desativado no painel administrativo."});
    ...
    if(totalPedido < Math.max(0,Number(configPagamento.valorMinimo)||0)) return res.status(400).json({erro:true,mensagem:`O valor mínimo para pagamento é ${Number(configPagamento.valorMinimo).toLocaleString("pt-BR",{style:"currency",currency:"BRL"})}.`});
...
app.post("/criar-pagamento", async (req, res) => {
  try {
    const configPagamento = await carregarConfigPagamentoServidorMS();
    if(configPagamento.cartaoAtivo === false) return res.status(400).json({erro:true,mensagem:"O pagamento com cartão está desativado no painel administrativo."});
